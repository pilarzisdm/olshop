...
def get_shops_in_location(location):
    ...
    if response.status_code == 200:
        ...
        for shop in shops:
            shop_name = shop.find('div', class_='shop-item__title').text.strip()  # Update class name
            products = shop.find_all('div', class_='product-item')  # Adjust the class based on the website structure

            print(f'Shop: {shop_name}')
            for product in products:
                product_name = product.find('div', class_='product-item__title').text.strip()  # Update class name
